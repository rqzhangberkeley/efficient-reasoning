...
class CustomGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.args.eval_strategy != "no":
            ...
            possible_values_eval = [n_gen for n_gen in range(1, global_batch_size + 1) if (global_batch_size) % n_gen == 0]
            if self.num_generations_eval not in possible_values_eval:
                raise ValueError(
                    f"The global eval batch size ({num_processes} x {args.per_device_eval_batch_size}) must be evenly "
                    f"divisible by the number of generations per prompt ({self.num_generations_eval}). Given the current "
                    f"eval batch size, the valid values for the number of generations are: {possible_values_eval}."
                )
        ...
        # Initialize completion data list. Added by RZ.
    ...
    def _generate_and_score_completions(
        ...
        if self.args.use_vllm:
            ...
            #########################################################
            all_prompts_text = gather_object(prompts_text) # RZ: len = num of responses per step (global sz)
            if self.accelerator.is_main_process:
                # Since 'prompts' contains 'num_generations' duplicates, we first take unique prompts, and generate
                # num_generations outputs for each one. This is faster than generating outputs for each duplicate
                # prompt individually.
                ordered_set_of_prompts = list(dict.fromkeys(all_prompts_text)) # RZ: len = num of unique prompts per step.

                if mode == 'train':
                    all_outputs = self.llm.generate(
                        ordered_set_of_prompts, sampling_params=self.sampling_params, use_tqdm=False
                    ) # RZ: List[RequestOutput], len = number of unique prompts.
                elif mode == 'eval':
                    all_outputs = self.llm.generate(
                        ordered_set_of_prompts, sampling_params=self.sampling_params_eval, use_tqdm=False
                    ) # RZ: List[RequestOutput], len = number of unique prompts.

                completion_ids = []
                for outputs in all_outputs:
                    for output in outputs.outputs:
                        completion_ids.append(output.token_ids) # RZ: List[List[int]], len = number of completions.
            ...
            completion_ids = completion_ids[process_slice] # RZ: len = train_batch_size_per_device. Here we use 32 for 1.5b model.
            ...
            raise NotImplementedError("Regular generation path is not implemented yet.")
            # RZ: We comment the non-vllm part for now.
        ...
        #########################################################
        # RZ: Prepare the masking for the logits.
        #########################################################
        # Now mask everything after the first EOS token
        ...
        #########################################################
        # RZ: End preparing the masking for the logits.
        #########################################################

        #########################################################
        # RZ: Now we first decode the generated completions, and then compute the rewards.
        #########################################################
        ...
        if is_conversational(inputs[0]):
            ...
            for prompt, completion in zip(prompts, completions_text):
                ...
                completions.append([{"role": "assistant", "content": bootstrap + completion}]) # RZ: all completions that are in thiss device. Length = train_batch_size_per_device.
        else:
            completions = completions_text

        # Compute rewards before masking EOS tokens
        ...
        # if self.accelerator.process_index == 0:  # Check if this is GPU0
        ...
        # if self.accelerator.process_index == 0:  # Check if this is GPU0
        #     import pdb; pdb.set_trace()
        # RZ: shape = (num_of_responses, num_reward_funcs)
        # RZ:  If num_generations = 4 and we have 3 prompts, the rewards would be arranged like:
        # rewards_per_func = [
        #     # Prompt 1's completions
        #     [reward1_completion1, reward1_completion2, reward1_completion3, reward1_completion4],
        #     # Prompt 2's completions
        #     [reward1_completion5, reward1_completion6, reward1_completion7, reward1_completion8],
        #     # Prompt 3's completions
        #     [reward1_completion9, reward1_completion10, reward1_completion11, reward1_completion12]
        # ]
        
        #########################################################
        # RZ: Compute the logits for the reference model.
        #########################################################
        with torch.inference_mode():
        ...
        #########################################################
        # RZ: Perpare the final rewards for training
        #########################################################

        # Apply weights to each reward function's output and sum
        rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).sum(dim=1)

        # Get the number of generations to use based on training vs eval mode
        local_num_generations = self.num_generations_eval if self.control.should_evaluate else self.num_generations

        # Compute grouped-wise rewards using the appropriate number of generations
        mean_grouped_rewards = rewards.view(-1, local_num_generations).mean(dim=1)
        std_grouped_rewards = rewards.view(-1, local_num_generations).std(dim=1)

        # Normalize the rewards to compute the advantages
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(local_num_generations, dim=0)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(local_num_generations, dim=0)
        ...
        return {
            ...
            "prompt_mask": prompt_mask,                  # (batch_size, prompt_length)
            "completion_ids": completion_ids,            # (batch_size, completion_length)
            "completion_mask": completion_mask,          # (batch_size, completion_length)
            "old_per_token_logps": old_per_token_logps, # (batch_size, completion_length)
            "ref_per_token_logps": ref_per_token_logps, # (batch_size, completion_length)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict: Optional[dict] = None):
        # First do the normal save
        super()._save(output_dir, state_dict)
        
        # Log all completions at the end of training
        if self.accelerator.is_main_process:
            import pandas as pd
